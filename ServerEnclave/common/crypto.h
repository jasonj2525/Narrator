...
public:
    ...
    void set_aes_key(uint8_t *aes_key) {
        memset(m_aes_key, 0, Aes_Key_Size);
        memcpy(m_aes_key, aes_key, Aes_Key_Size);
    }
    ...
    void copy_ecdsa_pubkey_key(uint8_t *ecdsa_public_key) {
        memcpy(ecdsa_public_key, m_ecdsa_public_key, sizeof(m_ecdsa_public_key));
    }

    // copy ecdsa public key
    void copy_ecdsa_pri_key(uint8_t *ecdsa_prikey_key) {
